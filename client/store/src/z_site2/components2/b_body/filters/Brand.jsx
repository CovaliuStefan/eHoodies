    ...
    return(
        <div className="BrandFilters">
            ...
            {brands.map((item,index)=>{
                    return(
                        <Link key={index} className={getProp(queries,"brandCat")?getProp(queries,"brandCat").includes(`-${brands[index]}-`)?"a activee":"a":"a"} to={`/2/products/${addMiniProp(queries,"brandCat",`-${brands[index]}-`)}`}>{brands[index]}</Link>//Multiple choices
                        // <Link key={index} className={getProp(queries,"brandCat")?getProp(queries,"brandCat").includes(`-${brands[index]}-`)?"a activee":"a":"a"} to={getProp(queries,"brandCat")===`-${brands[index]}-`?`/2/products/${removeProp(queries,"brandCat")}`:`/2/products/${addProp(queries,"brandCat",`-${brands[index]}-`)}`}>{brands[index]}</Link> //Single choice
